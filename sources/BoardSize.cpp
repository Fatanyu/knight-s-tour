...
bool BoardSize::hasValidSize() const
{
	...
	if (this->hasHeightOrWidthSmallerThanThree()) return false;

	// 3x1, 3x2, 3x3, 3x5, 3x6
	if (this->hasHeightThreeAndHaveZeroSolutions()) return false;

	// 1x3, 2x3, 3x3, 5x3, 6x3
	if (this->hasWidthThreeAndHaveZeroSolutions()) return false;

	// 4x1, 4x2, 4x4
	if (this->hasHeightFourAndHaveZeroSolutions()) return false;

	// 1x4, 2x4, 4x4
	if (this->hasWidthFourAndHaveZeroSolutions()) return false;
